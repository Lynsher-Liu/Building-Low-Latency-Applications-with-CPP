...
namespace Trading {
  MarketDataConsumer::MarketDataConsumer(Common::ClientId client_id, Exchange::MEMarketUpdateLFQueue *market_updates,
    ...
    ASSERT_MSG(incremental_mcast_socket_.init(incremental_ip, iface, incremental_port, /*is_listening*/ true) >= 0,
           "Unable to create incremental mcast socket. error:" + std::string(std::strerror(errno)));

    ASSERT_MSG(incremental_mcast_socket_.join(incremental_ip),
  ...
  auto MarketDataConsumer::startSnapshotSync() -> void {
    ...
    ASSERT_MSG(snapshot_mcast_socket_.init(snapshot_ip_, iface_, snapshot_port_, /*is_listening*/ true) >= 0,
           "Unable to create snapshot mcast socket. error:" + std::string(std::strerror(errno)));
    ASSERT_MSG(snapshot_mcast_socket_.join(snapshot_ip_), // IGMP multicast subscription.
