...
description: "Latency Optimization workflows for quantitative research, implementation, and production controls. use when tasks involve tail-latency compression and deterministic path behavior."
---

# Latency Optimization
## objective
Execute latency optimization work with reproducible research, explicit controls, and deployable outputs.
...
## outputs
- run `python scripts/latency_optimization_diagnostics.py input.csv --output diagnostics.json` and keep the json artifact.
- write an implementation memo using `references/latency-optimization-playbook.md` with assumptions, tests, limits, and rollout plan.

## resources
- use `scripts/latency_optimization_diagnostics.py` for deterministic diagnostics.
- use `references/latency-optimization-playbook.md` for the domain-specific checklist and delivery structure.
