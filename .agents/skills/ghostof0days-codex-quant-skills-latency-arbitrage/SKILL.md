...
description: "Latency Arbitrage workflows for quantitative research, implementation, and production controls. use when tasks involve tail-latency compression and deterministic path behavior."
---

# Latency Arbitrage
## objective
Execute latency arbitrage work with reproducible research, explicit controls, and deployable outputs.
...
## outputs
- run `python scripts/latency_arbitrage_diagnostics.py input.csv --output diagnostics.json` and keep the json artifact.
- write an implementation memo using `references/latency-arbitrage-playbook.md` with assumptions, tests, limits, and rollout plan.

## resources
- use `scripts/latency_arbitrage_diagnostics.py` for deterministic diagnostics.
- use `references/latency-arbitrage-playbook.md` for the domain-specific checklist and delivery structure.
